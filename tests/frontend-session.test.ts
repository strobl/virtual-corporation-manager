...
describe('local browser session recovery', () => {
  it('refreshes a stale session token and retries only the rejected request once', async () => {
    ...
    expect(await request('/api/preview', { baseRevision: 1, commands: [] })).toEqual({
      id: 'preview',
    });
    expect(fetch).toHaveBeenCalledTimes(4);
    expect(fetch.mock.calls[1]?.[1].headers['X-GitFlash-Token']).toBe('before-restart');
    expect(fetch.mock.calls[3]?.[1].headers['X-GitFlash-Token']).toBe('after-restart');
  });
