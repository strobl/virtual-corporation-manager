...
function companyOf100(): WorkspaceState {
  ...
  return {
    ...
    agents: Array.from({ length: 100 }, (_, i) => ({
      ...
      instructions: "",
      responsibilities: [
        i === 99 ? "Verify the packed artifact" : "Complete assigned work",
      ],
...
describe("local frontend organization adapter", () => {
  it("preserves all 100 agents and their department and reporting identities", () => {
    ...
    expect(snapshot.members.find((row) => row.id === "agent-99")).toMatchObject(
      { department: "Department 9", managerId: "agent-90", kind: "agent" },
    );
  ...
  it("never substitutes an archived or ended assignment into configured company counts", () => {
    ...
    expect(toSnapshot(state).members.some((row) => row.id === "agent-0")).toBe(
      false,
    );
    expect(
      toSnapshot(state).assignments.some(
        (row) => row.teamMemberId === "agent-1",
      ),
    ).toBe(false);
  });
  ...
  it("resolves company context from stable department and agent IDs", () => {
    const state = companyOf100();
    expect(
      companyForSelection(state, { kind: "department", id: "dept-9" }),
    ).toBe("company");
    expect(companyForSelection(state, { kind: "agent", id: "agent-99" })).toBe(
      "company",
    );
  });
  ...
  it("shows a shared agent at company level when its department belongs elsewhere", () => {
    ...
    const second = organizationTree(state).find(
      (row) => row.id === "company:second",
    );
    ...
    expect(selectedTreeRows(organizationTree(state), selection)).toEqual(["company:second/agent:agent-99"]);
  });
