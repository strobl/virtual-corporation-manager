...
function open() {
  const directory = mkdtempSync(join(tmpdir(), 'gitflash-undo-preview-'));
...
const company = (name: string): DomainCommand => ({
  type: 'company.create',
  input: { name, shortCode: name, description: 'Original purpose', color: '#123456' },
});
const agent = (companyId: string): DomainCommand => ({
  type: 'agent.create',
  companyId,
  input: {
    name: 'Analyst',
    ...
    kind: 'agent',
    instructions: 'Cite the supporting evidence.',
    responsibilities: ['Review customer needs'],
    departmentId: null,
    managerId: null,
  },
});
...
describe('reviewed undo', () => {
  it('previews exact reverse field values without writing a preview, audit, or configuration record', () => {
    ...
    apply(store, [
      {
        ...
        input: {
          ...
          output: 'Useful private result stays unchanged.',
          provenance: 'manual',
          status: 'submitted',
          durationMs: null,
          runId: null,
    ...
    const changed = apply(store, [
      { type: 'company.update', id: companyId, input: { description: 'Changed purpose' } },
      {
        ...
        id: agentId,
        input: {
          instructions: 'Use the revised evidence.',
          responsibilities: ['Review scope', 'Explain limitations'],
        },
      },
    ]);
    const before = store.snapshot();
    const persistedBefore = persistedRows(directory);
    const preview = store.previewUndo(changed.changeId, changed.state.revision);
    expect(preview).toMatchObject({
      changeId: changed.changeId,
      baseRevision: changed.state.revision,
    });
    ...
    expect(effects).toContain(
      'Instructions: "Use the revised evidence." → "Cite the supporting evidence."',
    );
    expect(effects).toContain(
      'Responsibilities: ["Review scope","Explain limitations"] → ["Review customer needs"]',
    );
    expect(effects).not.toContain('assignment');
    expect(effects).not.toContain('Retained deliverable');
    expect(effects).not.toContain('Useful private result');
    expect(store.snapshot()).toEqual(before);
    expect(persistedRows(directory)).toEqual(persistedBefore);
    ...
    expect(restored.work).toEqual(original.work);
    expect(restored.revision).toBe(changed.state.revision + 1);
  ...
  it('shows removal of a newly created agent and assignment while excluding unchanged companies', () => {
    const { store } = open();
    const original = apply(store, [company('STUDIO')]).state;
    ...
    expect(preview.changes[1]).toContain('Remove assignment');
    expect(preview.changes[1]).toContain('Primary: true');
  ...
  it('reviews and restores archive effects on status, dated assignments, and relationships', () => {
    const { store } = open();
    const companies = apply(store, [company('STUDIO'), company('PARTNER')]).state.companies;
    ...
    const original = store.snapshot();
    const archived = apply(store, [{ type: 'company.archive', id: companies[0]!.id }]);
    const preview = store.previewUndo(archived.changeId, archived.state.revision);
    expect(
      preview.changes.filter((line) => line.includes('Status: "archived" → "active"')),
    ).toHaveLength(2);
    const assignment = preview.changes.find((line) => line.startsWith('Restore assignment'))!;
    expect(assignment).toContain(
      `Ended at: ${JSON.stringify(archived.state.assignments[0]!.endedAt)} → null`,
    );
    const relationship = preview.changes.find((line) => line.startsWith('Restore relationship'))!;
    expect(relationship).toContain(
      `Ended at: ${JSON.stringify(archived.state.relationships[0]!.endedAt)} → null`,
    );
    expect(preview.changes.some((line) => line.startsWith('Restore company "PARTNER"'))).toBe(
      false,
    );
  ...
  it('rejects a stale confirmation after another change and preserves the intervening state', () => {
    const { store } = open();
    const created = apply(store, [company('STUDIO')]);
    const preview = store.previewUndo(created.changeId, created.state.revision);
    const intervening = apply(store, [company('PARTNER')]).state;
    expect(() => store.undo(preview.changeId, preview.baseRevision)).toThrow(
      expect.objectContaining({ code: 'STALE_PREVIEW' }),
    );
    expect(() => store.previewUndo(preview.changeId, preview.baseRevision)).toThrow(
      expect.objectContaining({ code: 'STALE_PREVIEW' }),
    );
    expect(() => store.previewUndo(preview.changeId, intervening.revision)).toThrow(
      expect.objectContaining({ code: 'UNDO_UNAVAILABLE' }),
    );
  ...
  it('uses the same eligibility checks for missing, permanent, and already undone changes', () => {
    const { store } = open();
    expect(() => store.previewUndo('missing', 0)).toThrow(
      expect.objectContaining({ code: 'NOT_FOUND' }),
    );
    expect(() => store.previewUndo('missing', -1)).toThrow(
      expect.objectContaining({ code: 'INVALID_INPUT' }),
    );
    const companyId = apply(store, [company('STUDIO')]).state.companies[0]!.id;
    const created = apply(store, [agent(companyId)]);
    const agentId = created.state.agents[0]!.id;
    const work = apply(store, [
      {
        type: 'work.record',
        input: {
          companyId,
          ...
          output: 'A complete useful result',
          provenance: 'codex',
          status: 'submitted',
          durationMs: 10,
          runId: 'undo-test-work',
        },
      },
    ]);
    expect(() => store.previewUndo(work.changeId, work.state.revision)).toThrow(
      expect.objectContaining({ code: 'UNDO_UNAVAILABLE' }),
    );
    const update = apply(store, [
      { type: 'company.update', id: companyId, input: { name: 'Revised studio' } },
    ]);
    const preview = store.previewUndo(update.changeId, update.state.revision);
    const restored = store.undo(preview.changeId, preview.baseRevision);
    expect(() => store.previewUndo(update.changeId, restored.revision)).toThrow(
      expect.objectContaining({ code: 'UNDO_UNAVAILABLE' }),
    );
    expect(() => store.previewUndo(restored.history[0]!.id, restored.revision)).toThrow(
      expect.objectContaining({ code: 'UNDO_UNAVAILABLE' }),
    );
