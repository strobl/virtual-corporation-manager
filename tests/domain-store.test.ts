...
function directory() {
  const dir = mkdtempSync(join(tmpdir(), 'gitflash-domain-'));
  directories.push(dir);
  return dir;
}
function open(dir = directory()) {
...
const agent = (companyId: string, name = 'Builder'): DomainCommand => ({
  type: 'agent.create',
  companyId,
  input: {
    name,
    role: 'Engineer',
    kind: 'agent',
    instructions: 'Produce verified implementation evidence.',
    responsibilities: ['Deliver the assigned task'],
    departmentId: null,
    managerId: null,
  },
});
...
describe('local SQLite company store', () => {
  it('persists preview separately, applies once, and retains state and replay after restart', () => {
    ...
    expect(result.state.companies[0]?.name).toBe('STUDIO');
    expect(result.state.revision).toBe(1);
    const replay = restarted.apply(preview.id);
    expect(replay.replayed).toBe(true);
    expect(replay.changeId).toBe(result.changeId);
    expect(replay.state.revision).toBe(1);
    apply(restarted, [agent(result.state.companies[0]!.id)]);
    restarted.close();
    const recovered = open(dir).snapshot();
    expect(recovered.agents).toHaveLength(1);
    expect(recovered.assignments[0]?.isPrimary).toBe(true);
  });

  it('refuses stale and invalid atomic previews without partial writes', () => {
    const store = open();
    const a = store.preview([company('A')], 0);
    const b = store.preview([company('B')], 0);
    store.apply(a.id);
    expectCode(() => store.apply(b.id), 'STALE_PREVIEW');
    const before = store.snapshot();
    expectCode(() => store.preview([company('C'), agent('missing')], before.revision), 'NOT_FOUND');
  ...
  it('undoes the latest change atomically with a monotonic revision and retained audit', () => {
    const store = open();
    const first = apply(store, [company('A')]);
    const second = apply(store, [agent(first.state.companies[0]!.id)]);
    expectCode(() => store.undo(first.changeId, 2), 'UNDO_UNAVAILABLE');
    const restored = store.undo(second.changeId, 2);
    ...
    expect(restored.history[0]?.action).toBe('change.undo');
    expect(restored.history.some((h) => h.undoable)).toBe(false);
  ...
  it('enforces ownership cycles, incoming totals and self/duplicate edges', () => {
    const store = open();
    const s = apply(store, [company('A'), company('B'), company('C')]).state;
    const [a, b, c] = s.companies.map((x) => x.id);
    const edge = (from: string, to: string, percentage: number | null): DomainCommand => ({
    ...
    apply(store, [edge(c!, b!, 40)]);
    expect(store.snapshot().relationships).toHaveLength(2);
  });

  it('preserves dated assignment history and rejects manager cycles', () => {
    const store = open();
    const s = apply(store, [company('A'), company('B')]).state;
    const [a, b] = s.companies.map((x) => x.id);
    const team = apply(store, [agent(a!, 'Lead'), agent(a!, 'Builder')]).state.agents;
    const [lead, builder] = team.map((x) => x.id);
    apply(store, [{ type: 'agent.update', id: builder!, input: { managerId: lead! } }]);
    expectCode(
      () => store.preview([{ type: 'agent.update', id: lead!, input: { managerId: builder! } }], 3),
      'MANAGER_CYCLE',
    );
    apply(store, [{ type: 'assignment.primary', agentId: builder!, companyId: b! }]);
    const rows = store.snapshot().assignments.filter((x) => x.agentId === builder);
    expect(rows).toHaveLength(3);
    expect(rows.filter((x) => x.isPrimary && !x.endedAt)).toHaveLength(1);
    expectCode(
      () =>
        store.preview(
          [{ type: 'assignment.end', id: rows.find((x) => x.isPrimary && !x.endedAt)!.id }],
          4,
        ),
      'PRIMARY_REQUIRED',
    );
  ...
  it('archives company relations, preserves multi-company members and clears dangling managers', () => {
    ...
    const team = apply(store, [agent(a!, 'Lead'), agent(a!, 'Builder')]).state.agents;
    apply(store, [
      { type: 'assignment.add', agentId: team[1]!.id, companyId: b! },
      { type: 'agent.update', id: team[1]!.id, input: { managerId: team[0]!.id } },
    ]);
    const state = apply(store, [{ type: 'company.archive', id: a! }]).state;
    expect(state.agents[0]?.status).toBe('archived');
    expect(state.agents[1]?.status).toBe('active');
    expect(state.agents[1]?.managerId).toBeNull();
    expect(
      state.assignments.find((x) => x.agentId === team[1]!.id && x.companyId === b && !x.endedAt)
        ?.isPrimary,
    ).toBe(true);
    apply(store, [
      { type: 'company.restore', id: a! },
      { type: 'agent.restore', id: team[0]!.id, companyId: a! },
    ]);
  ...
  it('imports a realistic 100-agent template twice with fresh IDs and no synthetic work', () => {
    const store = open();
    const definition = getTemplate('studio-100');
    const first = apply(store, [{ type: 'definition.import', definition }]).state;
    expect(first.agents).toHaveLength(100);
    expect(first.departments).toHaveLength(10);
    expect(first.work).toHaveLength(0);
    ...
    expect(first.agents.find((a) => a.role === 'SQLite Engineer')?.instructions).toContain(
      'transactions, constraints, indexes',
    );
    const second = apply(store, [{ type: 'definition.import', definition }]).state;
    expect(second.agents).toHaveLength(200);
  ...
  it('rejects malformed definitions and unknown commands without reserving a partial template', () => {
    const store = open();
    const definition = getTemplate('studio-20');
    definition.assignments[0]!.companyId = 'missing';
    expectCode(
      () => store.preview([{ type: 'definition.import', definition }], 0),
      'INVALID_REFERENCE',
    );
    expect(store.snapshot().revision).toBe(0);
    expect(store.snapshot().companies).toHaveLength(0);
    expectCode(
      () => validateDefinition({ ...definition, schemaVersion: 99 }),
      'UNSUPPORTED_SCHEMA',
    );
    expectCode(
      () => store.preview([{ type: 'anything' } as unknown as DomainCommand], 0),
      'UNKNOWN_COMMAND',
    );
    expectCode(
      () => validateDefinition({ ...getTemplate('studio-20'), work: [] }),
      'INVALID_DEFINITION',
    );
    const unsupported = getTemplate('studio-20');
    Object.assign(unsupported.agents[0]!, { apiKey: 'not-a-real-secret' });
  ...
  it('records real supplied work with provenance, idempotency and acceptance semantics', () => {
    const store = open();
    const s = apply(store, [company('A')]).state;
    const cid = s.companies[0]!.id;
    const aid = apply(store, [agent(cid)]).state.agents[0]!.id;
    const command: DomainCommand = {
      ...
      input: {
        ...
        agentId: aid,
        title: 'Review result',
        output: 'Checked the three specified routes. Route /health returned 200.',
        provenance: 'manual',
        status: 'submitted',
        durationMs: null,
        runId: 'test-real-run',
    ...
    const result = apply(store, [command]);
    expect(result.state.work[0]?.provenance).toBe('manual');
    expect(result.state.history[0]?.undoable).toBe(false);
    expectCode(() => store.undo(result.changeId, 3), 'UNDO_UNAVAILABLE');
    expectCode(() => store.preview([command], 3), 'DUPLICATE_RUN');
    apply(store, [{ type: 'work.accept', id: result.state.work[0]!.id }]);
    expect(store.snapshot().work[0]?.status).toBe('accepted');
    expectCode(
      () => store.preview([{ type: 'work.accept', id: result.state.work[0]!.id }], 4),
      'INVALID_STATE',
    );
  });
  ...
  it('backs up a live WAL workspace, validates restoration, and preserves recovery evidence', async () => {
    const dir = directory();
    const store = open(dir);
    apply(store, [company('A')]);
    const ledger = new DatabaseSync(join(dir, 'workspace.sqlite'));
    ledger.prepare('INSERT INTO integration_runs VALUES (?,?,?,?)').run(
      'run-a',
      'request-a',
      JSON.stringify({
        id: 'run-a',
        requestId: 'request-a',
        output: 'Saved original result',
        status: 'completed',
      }),
      '{}',
    );
    const backup = join(dir, 'first.sqlite');
    ...
    expect(restored.snapshot().companies.map((c) => c.name)).toEqual(['A']);
    expect(readdirSync(join(dir, 'backups')).some((f) => f.startsWith('before-restore-'))).toBe(
      true,
    );
    const recoveredLedger = new DatabaseSync(join(dir, 'workspace.sqlite'));
    expect(
      recoveredLedger
        .prepare('SELECT id FROM integration_runs')
        .all()
        .map((r) => r.id),
    ).toEqual(['run-a']);
    expect(recoveredLedger.prepare('SELECT info FROM integration_runs').get()?.info).toContain(
    ...
    await expect(restoreWorkspaceBackup(dir, corrupt)).rejects.toMatchObject({
      code: 'INVALID_BACKUP',
    });
    expect(open(dir).snapshot().companies).toHaveLength(1);
  });
  ...
  it('journals migrations, backs up before upgrade, and enforces SQLite foreign keys', () => {
    ...
    const old = new DatabaseSync(join(dir, 'backups', backups[0]!));
    ...
    expect(() =>
      checkDb
        .prepare('INSERT INTO departments VALUES (?,?,?,?,?)')
        .run('bad', 'missing', 'No company', '', null),
    ).toThrow();
    checkDb.close();
  ...
  it('refuses a second process store and recovers a stale process lock', () => {
    const dir = directory();
    const store = open(dir);
    expectCode(() => createWorkspaceStore(dir), 'WORKSPACE_BUSY');
    store.close();
    writeFileSync(
    ...
    expect(recovered.snapshot().revision).toBe(0);
  ...
  it('refuses a newer workspace schema without rewriting its database', () => {
    ...
    const store = open(dir);
    store.close();
    const future = new DatabaseSync(join(dir, 'workspace.sqlite'));
    future.exec('PRAGMA user_version=999;');
    future.close();
    const original = readFileSync(join(dir, 'workspace.sqlite'));
    expectCode(() => createWorkspaceStore(dir), 'UPGRADE_REQUIRED');
    expect(readFileSync(join(dir, 'workspace.sqlite'))).toEqual(original);
  });

  it('records completion after an agent was archived and preserves work during later configuration undo', () => {
    const store = open();
    const companyId = apply(store, [company('A')]).state.companies[0]!.id;
    ...
    apply(store, [
      {
        ...
        input: {
          ...
          output: 'The requested analysis finished after archival.',
          provenance: 'codex',
          status: 'submitted',
          durationMs: 10,
          runId: 'archived-completion',
        },
      },
    ]);
    const config = apply(store, [company('B')]);
    store.undo(config.changeId, config.state.revision);
    expect(store.snapshot().work).toHaveLength(1);
    expect(store.snapshot().work[0]?.runId).toBe('archived-completion');
  });
  ...
  it('previews exact field changes and full role instructions before any mutation', () => {
      ...
      [
        {
          ...
          input: {
            name: 'Studio',
            shortCode: 'STUDIO',
            description: 'Build useful local software.',
            color: '#123456',
          },
        },
      ],
    ...
    expect(preview.changes.join('\n')).toContain('Purpose: Build useful local software.');
    expect(preview.changes.join('\n')).toContain('Short code: STUDIO');
    const companyId = store.apply(preview.id).state.companies[0]!.id;
    const update = store.preview(
      [
        {
          type: 'company.update',
          id: companyId,
          input: { description: 'Deliver reviewed products.' },
        },
      ],
      1,
    );
    expect(update.changes.join('\n')).toContain(
      'Build useful local software. → Deliver reviewed products.',
    );
    const template = store.preview(
      [{ type: 'definition.import', definition: getTemplate('studio-100') }],
      1,
    );
    expect(
      template.changes.some(
        (line) => line.includes('Agent SQLite Engineer') && line.includes('Instructions:'),
      ),
    ).toBe(true);
  ...
  it('rolls back a failing migration and keeps its pre-upgrade backup', () => {
    ...
    expect(checked.prepare('PRAGMA user_version').get()?.user_version).toBe(1);
    expect(checked.prepare('SELECT name FROM companies').get()?.name).toBe('A');
    expect(checked.prepare('SELECT COUNT(*) AS n FROM schema_migrations').get()?.n).toBe(1);
    expect(
      checked.prepare("SELECT name FROM sqlite_master WHERE name='unique_work_run'").get(),
    ).toBeUndefined();
    checked.close();
  ...
  it('rejects a structurally invalid SQLite backup before replacing a working workspace', async () => {
    ...
    const store = open(dir);
    apply(store, [company('A')]);
    const backup = join(dir, 'invalid-ledger.sqlite');
    await store.backup(backup);
    store.close();
    const invalid = new DatabaseSync(backup);
    invalid.prepare('INSERT INTO integration_runs VALUES (?,?,?,?)').run(
      'run-a',
      'request-a',
      JSON.stringify({
        id: 'other-run',
  ...
  it('restores a valid backup over an unreadable current database and preserves original bytes', async () => {
    ...
    await store.backup(backup);
    store.close();
    const broken = Buffer.from('Simulated unreadable database bytes.');
    writeFileSync(join(dir, 'workspace.sqlite'), broken);
    await restoreWorkspaceBackup(dir, backup);
    expect(open(dir).snapshot().companies[0]?.name).toBe('A');
    const recovery = readdirSync(join(dir, 'backups')).find((name) =>
      name.startsWith('before-restore-unreadable-'),
    )!;
