...
export class CodexEventCollector {
  ...
  consume(line: string) {
    ...
    try {
      ...
      throw new IntegrationError(
        'runtime-protocol',
        'Codex returned an invalid event stream. Verify your CLI version.',
      );
    }
  ...
  result(code: number): ExecutionResult {
    if (code !== 0 || this.failed || !this.completed || !this.output.trim()) {
      throw new IntegrationError(
        'runtime-failed',
        'Codex did not return a successful task result. Check your CLI login and usage allowance, then retry.',
      );
    }
...
export async function codexStatus(env: NodeJS.ProcessEnv): Promise<IntegrationStatus['codex']> {
  ...
  if (!executable)
    return {
      ...
      state: 'unavailable',
      message:
        'Install the optional Codex CLI and run codex login. Company setup works without it.',
    };
  try {
    const login = await executeProcess(executable, ['login', 'status'], {
    ...
    return {
      ...
      message: authenticated
        ? 'Codex CLI is installed and reports a saved login. A task will use your existing runtime allowance.'
        : 'Run codex login in your terminal, then refresh. No model request has been made.',
    };
  } catch {
    return {
      available: true,
      ...
      message: 'Unable to confirm the CLI login. Run codex login status in your terminal.',
    };
  }
}
...
export async function executeCodex(
  ...
  if (!executable)
    throw new IntegrationError(
      'runtime-unavailable',
      'Codex CLI was not found. Install it and run codex login before starting a task.',
    );
  const childEnv = runtimeEnvironment(env);
  const help = await executeProcess(executable, ['exec', '--help'], {
    env: childEnv,
    timeoutMs: 5000,
    signal: input.signal,
  });
  ...
  const context = {
    ...
    agent: input.agent,
    colleagues: input.state.agents
      .filter((agent) =>
        input.state.assignments.some(
          (assignment) =>
            assignment.agentId === agent.id &&
            assignment.companyId === input.company.id &&
            !assignment.endedAt,
        ),
      )
      .map((agent) => ({
    ...
    priorWork: input.state.work
      .filter((work) => work.companyId === input.company.id && work.status !== 'failed')
      .slice(-5)
      .map((work) => ({
        title: work.title,
        agentId: work.agentId,
        output: work.output.slice(0, 20_000),
        status: work.status,
      })),
  };
  await writeFile(join(input.directory, 'company-context.json'), JSON.stringify(context, null, 2), {
    mode: 0o600,
  });
  const prompt = [
  ...
  const result = await executeProcess(
    ...
    ],
    {
      ...
      signal: input.signal,
      timeoutMs,
      onLine: (line) => collector.consume(line),
    },
  );
  const output = collector.result(result.code);
  const versionLabel = version.stdout.trim();
  output.runtimeVersion =
    version.code === 0 && /^codex-cli \d+\.\d+\.\d+(?:[-+][A-Za-z0-9.-]+)?$/.test(versionLabel)
      ? versionLabel
      : null;
  return output;
}
