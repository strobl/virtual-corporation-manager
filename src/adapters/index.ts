...
export function createIntegrationService(
  ...
  try {
    ...
    db.prepare('SELECT id, request_id, info, context FROM integration_runs LIMIT 0').all();
  } catch {
    db.close();
  ...
  const readRows = () =>
    db
      .prepare('SELECT id, info, context FROM integration_runs ORDER BY rowid')
      .all() as unknown as RunRow[];
  const save = (run: RunInfo) =>
    db
      .prepare('UPDATE integration_runs SET info = ? WHERE id = ?')
      .run(JSON.stringify(run), run.id);
  const getRun = (id: string): RunInfo | undefined => {
    const row = db.prepare('SELECT info FROM integration_runs WHERE id = ?').get(id) as
      { info: string } | undefined;
    return row ? (JSON.parse(row.info) as RunInfo) : undefined;
  };
  const runs = (): RunInfo[] =>
    readRows()
      .map((row) => JSON.parse(row.info) as RunInfo)
      .reverse();
  ...
  try {
    ...
    const rows = readRows().map((row) => {
      const run = JSON.parse(row.info) as RunInfo;
      ...
      if (
        ...
        throw new IntegrationError(
          'invalid-ledger',
          'The task ledger contains invalid records. Restore a validated workspace backup.',
        );
      }
      return run;
    });
    for (const run of rows) {
      ...
      const existingWork = store
        .snapshot()
        .work.find((work) => work.runId === run.id && work.status !== 'failed');
      if (existingWork) {
        run.status = 'completed';
        run.output = existingWork.output;
        run.completedAt = existingWork.createdAt;
    ...
    throw new IntegrationError(
      'invalid-ledger',
      'The task ledger could not be read safely. Restore a validated workspace backup.',
    );
  }
  ...
  async function deliverToSlack(run: RunInfo, origin: SlackOrigin) {
    ...
    run.deliveryReference = delivery.reference;
    save(run);
  }
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        const preview = store.preview(
          [
            {
              ...
              input: {
                ...
                agentId: run.agentId,
                title: run.task.slice(0, 160),
                output: run.output,
                provenance: origin ? 'slack' : run.transport,
                status: 'submitted',
          ...
          `Record ${run.agentName}'s completed task`,
        );
  ...
  async function processQueue() {
    while (!closed) {
      ...
      const deadline = setTimeout(() => controller.abort(), timeoutMs + 10_000);
      deadline.unref();
      try {
        ...
        const input = {
          ...
          onDispatch: (reference: string) => {
            run.deliveryReference = reference;
        ...
        const result =
          run.transport === 'buzz'
            ? await (options.executeBuzz ?? ((value) => executeBuzz(value, env, timeoutMs)))(input)
            : await (options.executeCodex ?? ((value) => executeCodex(value, env, timeoutMs)))(
                input,
              );
        if (!result.output.trim() || Buffer.byteLength(result.output) > 1_000_000)
          throw new IntegrationError(
            'invalid-result',
            'The runtime returned an empty or oversized result.',
          );
        run.output = result.output;
        run.outputSha256 = createHash('sha256').update(result.output).digest('hex');
        run.runtimeVersion = result.runtimeVersion;
        ...
        run.status = 'failed';
        run.error = redactError(error, env);
        if (error instanceof IntegrationError && error.code === 'delivery-uncertain')
          run.deliveryStatus = 'uncertain';
      } finally {
  ...
  async function enqueue(request: RunRequest, origin: SlackOrigin | null = null): Promise<RunInfo> {
    ...
    if (
      ...
      !request.task.trim() ||
      request.task.length > 30_000 ||
      typeof request.requestId !== 'string' ||
      !/^[A-Za-z0-9:_-]{8,200}$/.test(request.requestId) ||
      (request.transport && !['codex', 'buzz'].includes(request.transport))
    ) {
      throw new IntegrationError(
        'invalid-request',
        'Provide an agent, a task up to 30,000 characters, and a unique request ID (8–200 letters, digits, colons, underscores or hyphens).',
      );
    }
    const task = request.task.trim();
    const transport = request.transport ?? 'codex';
    const prior = db
      .prepare('SELECT info, context FROM integration_runs WHERE request_id = ?')
      .get(request.requestId) as { info: string; context: string } | undefined;
    if (prior) {
      const existing = JSON.parse(prior.info) as RunInfo;
      const existingContext = JSON.parse(prior.context) as RunContext;
      if (
        existing.agentId !== request.agentId ||
        existing.task !== task ||
        existing.transport !== transport ||
        JSON.stringify(existingContext.origin) !== JSON.stringify(origin)
      ) {
        throw new IntegrationError(
          'request-conflict',
          'This request ID already belongs to a different task. Generate a new request ID.',
        );
      }
      return existing;
    }
    ...
    const agent = state.agents.find(
      (item) => item.id === request.agentId && item.status === 'active' && item.kind === 'agent',
    );
    if (!agent)
      throw new IntegrationError('agent-not-found', 'Select an active AI agent for this task.');
    const assignments = state.assignments.filter(
      (item) => item.agentId === agent.id && !item.endedAt,
    );
    const assignment = assignments.find((item) => item.isPrimary) ?? assignments[0];
    const company = state.companies.find(
      (item) => item.id === assignment?.companyId && item.status === 'active',
    );
    if (!company)
      throw new IntegrationError(
        'company-not-found',
        'Assign the agent to an active company before starting work.',
      );
    if (runs().filter((run) => run.status === 'queued' || run.status === 'running').length >= 20)
      throw new IntegrationError(
        'queue-full',
        'The local task queue is full. Wait for a running task to finish.',
      );
    const run: RunInfo = {
      id: randomUUID(),
      ...
      durationMs: null,
      outputSha256: null,
      deliveryStatus: origin ? 'pending' : 'none',
      runtimeVersion: null,
      sessionId: null,
    ...
    db.prepare(
      ...
      run.requestId,
      JSON.stringify(run),
      JSON.stringify({ agent, company, state, origin } satisfies RunContext),
    );
    pump();
    return run;
  }
  ...
  return {
    async status(): Promise<IntegrationStatus> {
      if (!statusCache || Date.now() - statusCache.checkedAt > 15_000)
        statusCache = { value: await codexStatus(env), checkedAt: Date.now() };
      const all = runs();
      return {
        codex: statusCache.value,
        buzz: buzzStatus(env),
        slack: {
          state: slack.status(),
          message:
            slack.status() === 'connected'
              ? 'Socket Mode is connected. Only the configured owner and channel can submit tasks.'
              : 'Slack requires an installed app, app/bot tokens, and a configured owner/channel. Connect explicitly to receive tasks.',
        },
