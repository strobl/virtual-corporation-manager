...
export interface RunInfo {
  ...
  deliveryStatus: 'none' | 'pending' | 'sent' | 'uncertain' | 'failed';
...
export interface IntegrationStatus {
  codex: {
    ...
    state: 'unavailable' | 'authentication-required' | 'ready';
    message: string;
  };
...
export interface ExecutionInput {
  ...
  directory: string;
  signal: AbortSignal;
  onDispatch?: (reference: string) => void;
}
...
export function redactError(value: unknown, env: NodeJS.ProcessEnv = process.env): string {
  ...
  for (const [key, secret] of Object.entries(env)) {
    if (/(?:TOKEN|KEY|SECRET|PASSWORD|AUTH_TAG)/i.test(key) && secret && secret.length >= 6)
      message = message.split(secret).join('[redacted]');
  }
  return message
    .replace(/(?:xox[baprs]-|xapp-|sk-[A-Za-z0-9_-]*|nsec1)[A-Za-z0-9_-]+/g, '[redacted]')
