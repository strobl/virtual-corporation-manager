...
export function parseSelection(key: string): Selection | null {
  ...
  const context = key.includes("/") && key.startsWith("company:") ? key.split("/")[0]!.slice(8) : undefined;
  return (kind === "company" || kind === "department" || kind === "agent") &&
    id.join(":")
...
export function companyAgents(
  ...
  return state.agents.filter(
    (row) => row.status === "active" && ids.has(row.id),
  );
}
export function companyForSelection(
  ...
  if (selection?.kind === "department")
    return (
      state.departments.find((row) => row.id === selection.id)?.companyId ??
      null
    );
  if (selection?.kind === "agent") {
    const rows = activeAssignments(state).filter(
      (row) => row.agentId === selection.id,
    );
    if (selection.companyId && rows.some(row => row.companyId === selection.companyId)) return selection.companyId;
...
export function toSnapshot(state: WorkspaceState): WorkspaceSnapshot {
  const departments = new Map(
    state.departments.map((row) => [row.id, row.name]),
  );
  return {
    ...
    members: state.agents
      ...
      .map((row) => ({
        ...
        department: row.departmentId
          ? (departments.get(row.departmentId) ?? null)
          : null,
      })),
  ...
  return state.companies
    ...
    .flatMap((company) => {
      ...
      const agentNodes = (
        ...
        agents
          .filter(
            (agent) =>
              parentMatches ||
              matches(agent.name, agent.role, ...agent.responsibilities),
          )
          .map((agent) => ({
      ...
      const unassigned = agentNodes(
        all.filter(
          (agent) =>
            !agent.departmentId || !localDepartmentIds.has(agent.departmentId),
        ),
        wholeCompany,
      );
...
export function selectedTreeRows(
  nodes: TreeNode[],
  selection: Selection | null,
): string[] {
  if (!selection) return [];
  return nodes.flatMap((node) => {
    ...
    return [
      ...(entity?.kind === selection.kind && entity.id === selection.id && (!selection.companyId || entity.companyId === selection.companyId)
        ? [node.id]
        : []),
