...
GitFlash is one TypeScript application. A Node.js process serves a prebuilt React console on `127.0.0.1`. SQLite stores companies, departments, agents, dated assignments, relationships, work, reviewed changes and runtime receipts. There is no hosted database, account service, billing layer or required telemetry.
