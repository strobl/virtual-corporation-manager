...
**Buzz.xyz + Slack:** [Visual overview](integrations/README.md) · [Buzz team-import guide](integrations/buzz.md) · [Slack setup and app manifest](integrations/slack.md) · [Integration brand kit](https://github.com/strobl/virtual-corporation-manager/tree/main/brand/integrations)
...
This guide describes **`0.1.0-alpha.9` corporation management**. The primary [company-management loop](product/corporation-management.md) requires no runtime connection or executed task. Open **Tools → Connections** for optional runtime setup. **Tools → Agent runs & records** holds individual output and manual contributions, with **Workflow examples** as a separate tab. Historical alpha.5/alpha.6, platform and live integration evidence retains its original identity in [acceptance](acceptance.md); the versioned release manifest records the current source and package checks.
