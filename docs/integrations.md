...
Python 3.8+ and the fixed local Codex sandbox must pass a prerequisite check before any model stage. `GITFLASH_PYTHON_PATH` may select an absolute Python executable. Native Windows supports the local core and Time Tracker, but this fixed checker is unavailable there. macOS has an earlier standalone adapter/sandbox observation; Linux and the final candidate matrix require their own linked results in [acceptance evidence](acceptance.md). A successful Codex login alone does not establish workflow readiness on an operating system.
