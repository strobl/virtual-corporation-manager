...
Select an active company agent, or open **Work → Individual tasks**, enter the actual task and choose Run. VCM allows at most 20 outstanding individual tasks and executes one at a time. The subprocess runs in a dedicated task directory with a read-only sandbox. It receives the selected role, current company context and the five latest nonfailed company outputs. It returns its text deliverable through the supported JSONL event stream. The app saves a result only after successful process and terminal task completion. A submitted work record still needs human acceptance.
