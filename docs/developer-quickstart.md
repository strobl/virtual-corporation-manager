...
This guide targets the `0.1.0-alpha.6` branding maintenance release. Start with its verified GitHub release archive after publication and its matching source/checksum record. Check its [release acceptance](acceptance.md#branding-maintenance-release--6-september-2026). Published alpha.5 and earlier candidates are different artifacts. Follow [the README install block](../README.md#install-the-reviewed-archive); these commands assume the same `./vcm-preview` installation and working directory. Product Studio's actual useful-result acceptance remains open after two intake timeouts; see the [release evidence and limits](acceptance.md#technical-prerelease--6-september-2026).
