...
| ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| Browser console    | Setup, organization views, change review, work/jobs, Time Tracker and integration controls. Reads snapshots; retains pending Apply identity in session storage.                  | [App](../src/web/App.tsx), [client](../src/web/client.ts), [change recovery](../src/web/change-recovery.ts)  |
...
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Startup and identity   | Resolve `--data-dir`, then `GITFLASH_DATA_DIR`, then `~/.gitflash`; own shutdown and CLI recovery commands. `vcm` and compatible `gitflash` target the same binary. Public identity is VCM; package/data/wire identifiers retain compatibility. | [CLI](../src/cli/index.ts), [brand](../src/brand.ts), [package](../package.json)                                                                                                                                                            |
| Browser organization   | Setup drafts, corporation views, optional reporting hierarchy, company relationship map and contextual agent inspection.                                                                                                                        | [setup](../src/web/CorporationSetup.tsx), [workspace](../src/web/CorporationWorkspace.tsx), [org chart](../src/components/organization/CorporationOrgChart.tsx), [company map](../src/web/CompanyMap.tsx)                                   |
...
| Individual work        | Record supplied manual output or explicitly dispatch an optional task; retain run context, output and transport receipts. Work acceptance is a separate reviewed command.                                                                       | [Work](../src/web/Work.tsx), [coordinator](../src/adapters/index.ts), [Codex](../src/adapters/codex.ts), [Buzz](../src/adapters/buzz.ts), [Slack](../src/adapters/slack.ts)                                                                 |
...
Individual execution captures the selected company/agent context before adapter dispatch. The queue admits at most 20 active-plus-queued requests and runs one at a time. Request identity prevents duplicate dispatch; output and owner acceptance are separate records.

PS-001 captures five distinct roles, a named owner and pinned inputs/content. After local prerequisite checks, intake → requirements → build → QA → handoff run sequentially in distinct observed sessions. Actual files are captured; reply code blocks are not artifacts. A fixed Python oracle checks the candidate and a different QA session records criteria and observed commands. There is one active workflow and at most three active-plus-queued jobs, including retries. Start/Retry admission is transactional; refusal consumes no retry request or allowance. A successful retry's replay remains idempotent when the queue is full. Ordinary repairs and explicit runtime retries are each bounded at two. The separate individual-task queue may run alongside this workflow.

At `waiting_owner`, the service seals the fourteen-entry ZIP with frozen pre-decision `PROVENANCE.json`. `job.bundle` stores its SHA-256 and byte length outside the ZIP, avoiding a self-referential hash. Explicit accept/reject validates the bytes and stores `ownerReview.bundleSha256` atomically with the decision. Later downloads and restored downloads remain byte-identical. The full evidence JSON separately includes the current owner decision. Historical decided records without these fields remain unbound; an old undecided job is sealed only when a new explicit decision occurs.
