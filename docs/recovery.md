...
These commands assume the VCM CLI is installed. The default workspace is `~/.gitflash`. Add the same `--data-dir <path>` to every command when using a different workspace.

## Save a complete workspace

Stop the running VCM server with **Ctrl+C**, then choose a new backup filename:

```sh
vcm backup --output ./vcm-backup.sqlite
```
...
Stop VCM, then restore the chosen file:

```sh
vcm restore --from ./vcm-backup.sqlite
vcm doctor
vcm --no-open
```
...
1. Stop VCM and make a backup with the command above.
2. Install the desired published VCM release.
3. Run `vcm doctor`, then start VCM and inspect the company and Work views.

Opening an older supported workspace runs its migrations automatically. VCM first saves a `backups/before-schema-*.sqlite` copy, then applies pending schema changes and journal entries in one transaction. A failed migration leaves the previous schema and data in place. The current release supports database schema 5 and refuses a newer schema rather than changing it.

To return to an earlier release, restore the backup created before that upgrade and use the compatible older VCM version. Installing old code over a newer database is not a downgrade procedure.
...
| Workspace lock cannot be read              | Confirm no VCM process is using this directory. Preserve the directory, then remove only its stale `workspace.lock`. Normal stale locks with a dead recorded PID are recovered automatically. |
| Newer schema or migration journal mismatch | Keep the database unchanged. Use its compatible release, upgrade VCM, or restore a known compatible backup.                                                                                   |
...
| Interrupted or failed task                 | Inspect the saved output and any external runtime activity before submitting a new request. VCM does not automatically repeat uncertain external work.                                        |
