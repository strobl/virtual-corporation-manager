...
export async function writeNotices(inputs) {
  ...
  for (const input of inputs) {
    ...
    const dir=full.slice(0,boundary)+segments.slice(0,segments[0].startsWith('@')?2:1).join('/');
  ...
  console.log(`Retained complete license notices for ${packages.size} bundled dependency packages.`);
}
